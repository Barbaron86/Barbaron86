"""Generate a GitHub contribution streak SVG card."""
...
def _is_retryable_http_error(error: HTTPError) -> bool:
    """Return whether an HTTP error is considered temporary."""
...
def github_graphql(
    ...
    """Execute a GitHub GraphQL request with retries for transient failures."""
...
def get_contributions_for_year(
    ...
    for week in weeks:
        for day in week["contributionDays"]:
            ...
            contributions[contribution_date] = int(
...
def get_all_contributions(
    ...
    first_year = get_account_created_year(
        token=token,
        username=username,
    )
    ...
    for year in range(
        ...
        logger.info(
            "Fetching GitHub contributions for %d",
            year,
        )

        year_contributions = get_contributions_for_year(
            token=token,
            username=username,
            year=year,
        )
    ...
    return {
        ...
        for contribution_date, count in contributions.items()
        if contribution_date <= today
    }


def _active_contributions(
    contributions: ContributionMap,
) -> ContributionMap:
...
def calculate_longest_streak(
    ...
    active_dates = sorted(
        _active_contributions(contributions)
    )
...
def calculate_first_contribution(
    contributions: ContributionMap,
) -> date | None:
    """Return the first date containing a contribution."""

    active = _active_contributions(
        contributions
    )
...
def calculate_current_year_contributions(
    ...
    return sum(
        count
        for contribution_date, count in contributions.items()
        if contribution_date.year == current_year
    )
...
def calculate_active_days(
    ...
    return sum(
        ...
        for contribution_date, count in contributions.items()
        if (
            contribution_date.year == current_year
            and count > 0
        )
    )
...
def calculate_best_day(
    ...
    best_day_date, best_day_count = max(
        active.items(),
        key=lambda item: (
            item[1],
            item[0],
    ...
    return best_day_date, best_day_count
...
def calculate_stats(
    ...
    return StreakStats(
        ...
        first_contribution=calculate_first_contribution(
            contributions=contributions,
        ),
        ...
        current_year_contributions=(
            calculate_current_year_contributions(
                contributions=contributions,
...
def format_period(
    ...
    return (
        f"{format_date(value=period.start)}"
        f" — "
        f"{format_date(value=period.end)}"
    )
...
def render_svg(
    ...
    if not TEMPLATE_PATH.exists():
        raise RuntimeError(
            f"SVG template not found: {TEMPLATE_PATH}"
        )
    ...
    raw_replacements = {
        ...
        "{{FIRST_CONTRIBUTION}}": format_date(
            value=stats.first_contribution,
        ),
        "{{CURRENT}}": str(
            stats.current.days
        ),
        "{{CURRENT_RANGE}}": format_period(
            period=stats.current,
        ),
        "{{LONGEST}}": str(
            stats.longest.days
        ),
        "{{LONGEST_RANGE}}": format_period(
            period=stats.longest,
        ),
        "{{THIS_YEAR}}": str(
            stats.current_year_contributions
        ),
        "{{CURRENT_YEAR}}": str(
            current_year
        ),
        ...
        "{{BEST_DAY_DATE}}": format_date(
            value=stats.best_day_date,
        ),
    }
    ...
    if remaining_placeholders:
        raise RuntimeError(
            "Unsubstituted SVG placeholders: "
            f"{remaining_placeholders}"
        )
...
def main() -> None:
    ...
    logger.info(
        "Generating GitHub streak for %s",
        username,
    )
    ...
    if not contributions:
        raise RuntimeError(
            "GitHub returned no contribution calendar data"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.exception(
            "GitHub streak generation failed"
        )
        sys.exit(1)
